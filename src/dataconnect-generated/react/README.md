...
- [**Queries**](#queries)
  ...
  - [*GetTicketOrdersStripeArtifactsForCallable*](#getticketordersstripeartifactsforcallable)
...
You can execute the `GetTicketOrdersStripeArtifactsForCallable` Query using the following Query hook function, which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts):

```javascript
useGetTicketOrdersStripeArtifactsForCallable(dc: DataConnect, vars: GetTicketOrdersStripeArtifactsForCallableVariables, options?: useDataConnectQueryOptions<GetTicketOrdersStripeArtifactsForCallableData>): UseDataConnectQueryResult<GetTicketOrdersStripeArtifactsForCallableData, GetTicketOrdersStripeArtifactsForCallableVariables>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useGetTicketOrdersStripeArtifactsForCallable(vars: GetTicketOrdersStripeArtifactsForCallableVariables, options?: useDataConnectQueryOptions<GetTicketOrdersStripeArtifactsForCallableData>): UseDataConnectQueryResult<GetTicketOrdersStripeArtifactsForCallableData, GetTicketOrdersStripeArtifactsForCallableVariables>;
```

### Variables
The `GetTicketOrdersStripeArtifactsForCallable` Query requires an argument of type `GetTicketOrdersStripeArtifactsForCallableVariables`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:

```javascript
export interface GetTicketOrdersStripeArtifactsForCallableVariables {
  orderIds: UUIDString[];
}
```
### Return Type
Recall that calling the `GetTicketOrdersStripeArtifactsForCallable` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `GetTicketOrdersStripeArtifactsForCallable` Query is of type `GetTicketOrdersStripeArtifactsForCallableData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
export interface GetTicketOrdersStripeArtifactsForCallableData {
  ticketOrders: ({
    id: UUIDString;
    stripeCheckoutSessionId?: string | null;
    stripePaymentIntentId?: string | null;
...
### Using `GetTicketOrdersStripeArtifactsForCallable`'s Query hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, GetTicketOrdersStripeArtifactsForCallableVariables } from '@dataconnect/generated';
import { useGetTicketOrdersStripeArtifactsForCallable } from '@dataconnect/generated/react'

export default function GetTicketOrdersStripeArtifactsForCallableComponent() {
  // The `useGetTicketOrdersStripeArtifactsForCallable` Query hook requires an argument of type `GetTicketOrdersStripeArtifactsForCallableVariables`:
  const getTicketOrdersStripeArtifactsForCallableVars: GetTicketOrdersStripeArtifactsForCallableVariables = {
    orderIds: ..., 
  };

  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useGetTicketOrdersStripeArtifactsForCallable(getTicketOrdersStripeArtifactsForCallableVars);
  // Variables can be defined inline as well.
  const query = useGetTicketOrdersStripeArtifactsForCallable({ orderIds: ..., });

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useGetTicketOrdersStripeArtifactsForCallable(dataConnect, getTicketOrdersStripeArtifactsForCallableVars);

  // You can also pass in a `useDataConnectQueryOptions` object to the Query hook function.
  const options = { staleTime: 5 * 1000 };
  const query = useGetTicketOrdersStripeArtifactsForCallable(getTicketOrdersStripeArtifactsForCallableVars, options);

  // You can also pass both a `DataConnect` instance and a `useDataConnectQueryOptions` object.
  const dataConnect = getDataConnect(connectorConfig);
  const options = { staleTime: 5 * 1000 };
  const query = useGetTicketOrdersStripeArtifactsForCallable(dataConnect, getTicketOrdersStripeArtifactsForCallableVars, options);

...
| `getTemplateSyncStatus` | 30 | 5 minutes | GOV.UK Notify template enumeration |
...
`getTemplateSyncStatus` fans out to one GOV.UK Notify `getTemplateById` call per configured email template (12 templates as of writing), not one call per invocation — its per-invocation external-API cost is comparable to `getAnnouncementTemplates`/`previewAnnouncementTemplate`, so it shares their 30/5 minute limit rather than being tighter. It was originally set to 10/5 minutes without that comparison, which was too tight for its actual usage pattern: an admin-only diagnostic tool with a single manual "check, fix, re-check" workflow and no auto-fetch, unlike the enumeration-risk callables at the top of this table (see #400).
...
| `getTemplateSyncStatus` | Medium | High | GOV.UK Notify | High | Admin + enabled; 30/5 minutes |

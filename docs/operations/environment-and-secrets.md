...
- Ticket order lifecycle emails (issue #186) use template keys `ticketOrderPaid`, `ticketOrderFailed`, and `ticketOrderRefunded`, which map to `GOV_NOTIFY_TEMPLATE_TICKET_ORDER_PAID`, `GOV_NOTIFY_TEMPLATE_TICKET_ORDER_FAILED`, and `GOV_NOTIFY_TEMPLATE_TICKET_ORDER_REFUNDED` respectively. Notify personalisation keys sent by Functions match the fields in `functions/src/paymentLifecycleEmailDispatcher.ts` (for example `customerFirstName`, `eventTitle`, `ticketTypeTitle`, `totalFormatted`, `orderStatusLabel`, `myPaymentsUrl`, and `refundFormatted` on the refund template).
